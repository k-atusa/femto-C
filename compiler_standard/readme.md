...
구조체의 접근 제어는 대문자 규칙에 더해 보호 규칙이 추가됩니다. 모듈 외부에서 구조체 타입을 사용하려면 대문자로 시작해야 합니다. 대문자로 시작하지 않는 구조체나 멤버는 같은 모듈 내부에서만 사용할 수 있습니다.
...
멤버 접근은 추가 보호를 걸 수 있습니다. 언더바로 시작하는 멤버는 해당 구조체 메서드에서만 접근할 수 있습니다. 메서드의 접근은 멤버 접근과 오나전히 동일한 방식입니다. 대문자 규칙과 언더바 보호 규칙을 따릅니다.
...
`template` 키워드로 템플릿 모듈을 선언하세요. 템플릿 모듈은 자체적으로 컴파일될 수 없고 템플릿 인자를 받아 호출되어야 합니다. 템플릿 선언은 모듈당 1회만 가능합니다.
